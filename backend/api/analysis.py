...
async def perform_new_analysis(request: NewAnalysisRequest):
    ...
    try:
        ...
        use_news = app_config.settings.get('USE_NEWSAPI', False) or app_config.settings.get('USE_CRYPTOCOMPARE_NEWS', False)
        ...
        logging.warning(f"Geçersiz sembol analizi denemesi: {unified_symbol}. Hata: {e}")
        raise HTTPException(status_code=404, detail=f"Geçersiz veya borsada bulunamayan sembol: {unified_symbol}")
