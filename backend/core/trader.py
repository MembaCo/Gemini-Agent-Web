...
def open_new_trade(symbol: str, recommendation: str, timeframe: str, current_price: float):
    ...
    if result.get("status") == "success":
        # DÜZELTME: Veritabanına, borsadan gelen gerçek gerçekleşme fiyatını kaydediyoruz.
        final_entry_price = result.get('fill_price', current_price)
        
        managed_position_details = {
            "symbol": symbol, "side": trade_side, "amount": trade_amount, 
            "entry_price": final_entry_price, # Gerçekleşme fiyatı kullanılıyor
        ...
        raise TradeException(f"Borsada işlem emri gönderimi başarısız oldu: {result.get('message')}")
...
def close_existing_trade(symbol: str, close_reason: str = "MANUAL"):
    ...
    # DÜZELTME: Kapatma emrinin sonucunu alıyoruz.
    ...
    if result.get("status") == "success":
        ...
        if closed_pos:
            # DÜZELTME: PNL hesaplaması için borsadan gelen gerçek kapanış fiyatını kullanıyoruz.
        ...
        raise TradeException(f"Pozisyon kapatılamadı. Borsa yanıtı: {result.get('message')}")