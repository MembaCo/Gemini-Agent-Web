...
export const AuthProvider = ({ children }) => {
    ...
    const apiFetch = useCallback(async (url, options = {}) => {
        ...
        if (response.status === 401) {
            // Yetkilendirme hatası, token geçersiz. Oturumu kapat.
            logout();
    ...
    const login = async (username, password) => {
        try {
            ...
            // Bu istek apiFetch kullanmaz çünkü henüz token yok
    ...
    // API çağrı fonksiyonlarını buraya taşıyoruz
    ...
    const runScanner = useCallback(() => apiFetch('/scanner/run', { method: 'POST' }), [apiFetch]);
