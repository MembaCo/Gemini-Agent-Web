...
// --- YARDIMCI BİLEŞENLER (Bu bileşenler değişmedi, aynı kalabilir) ---
const Toast = ({ message, type, onClose }) => {
  ...
  return ( <div className={`fixed bottom-5 right-5 bg-gray-800 border-l-4 ${borderColors[type]} text-white p-4 rounded-lg shadow-2xl flex items-center gap-4 z-50 animate-fade-in-up`}> {icons[type]}<p>{message}</p><button onClick={onClose} className="ml-auto text-gray-500 hover:text-white"><X size={18} /></button> </div> );
};

const Modal = ({ children, isVisible, onClose, maxWidth = 'max-w-md' }) => {
  if (!isVisible) return null;
  return ( <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-40" onClick={onClose}> <div className={`bg-gray-800 border border-gray-700 rounded-xl p-6 sm:p-8 w-full m-4 ${maxWidth}`} onClick={e => e.stopPropagation()}>{children}</div> </div> );
};

const Switch = ({ checked, onChange }) => ( <button type="button" onClick={() => onChange(!checked)} className={`${checked ? 'bg-blue-600' : 'bg-gray-600'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors`}> <span className={`${checked ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} /> </button> );

// --- ANA ARAYÜZ BİLEŞENLERİ (Bu bileşenler de değişmedi) ---
const Header = ({ appVersion, onSettingsClick, isLoading }) => ( <header className="mb-8 flex justify-between items-center"> <div> <h1 className="text-3xl md:text-4xl font-bold text-white">Gemini Trading Agent</h1> <p className="text-gray-400 mt-1">v{appVersion} - Canlı Performans Panosu</p> </div> <button onClick={onSettingsClick} disabled={isLoading} className="p-2 rounded-full hover:bg-gray-700 text-gray-400 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"> <Settings size={24} /> </button> </header> );
...
const SettingsModal = ({ settings, isVisible, onClose, onSave }) => {
    ...
    return ( <Modal isVisible={isVisible} onClose={onClose} maxWidth="max-w-3xl"><div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold text-white">Uygulama Ayarları</h2><button onClick={onClose} className="p-2 rounded-full hover:bg-gray-700 text-gray-400 hover:text-white"><X size={24} /></button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 max-h-[70vh] overflow-y-auto pr-4 text-sm">{Object.keys(editableSettings).length > 0 ? ( Object.keys(editableSettings).sort().map(key => ( <div key={key} className="flex justify-between items-center border-b border-gray-700 py-2 gap-4"><label className="text-gray-400 flex-shrink-0">{key}</label>{renderInput(key, editableSettings[key])}</div> ))) : ( <p className="text-gray-400 col-span-2">Ayarlar yükleniyor...</p> )}</div><div className="mt-6 flex justify-end"><button onClick={() => onSave(editableSettings)} className="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-md flex items-center gap-2"><Save size={18}/>Ayarları Kaydet</button></div></Modal> );
};
...
const Dashboard = () => {
  ...
  useEffect(() => {
      loadAllData(true);
      const interval = setInterval(() => loadAllData(false), 5000); // Fetch all data
  ...
  return (
    <div className="bg-gray-900 min-h-screen text-gray-300 font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <Header appVersion={settings.APP_VERSION || "3.2.1-stable"} onSettingsClick={() => setIsSettingsModalVisible(true)} isLoading={isLoading} />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8"><NewAnalysis onAnalysisStart={handleAnalysis} isAnalyzing={isAnalyzing} /><ProactiveScanner onRunScan={handleRunScan} isScanning={isScanning} /></div>
