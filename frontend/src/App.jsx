...
export default function App() {
  ...
  const fetchData = useCallback(async (showLoading = false) => {
    ...
    setError(null);
    try {
      const [dashboardRes, positionsRes, settingsRes] = await Promise.all([fetch(`${API_URL}/dashboard/stats`), fetch(`${API_URL}/positions/`), fetch(`${API_URL}/settings/`)]);
      if (!dashboardRes.ok || !positionsRes.ok || !settingsRes.ok) throw new Error('Backend API ile iletişim kurulamadı.');
  ...
  const handleAnalysis = useCallback(async ({ symbol, timeframe }) => {
    setIsAnalyzing(true);
    try {
      const res = await fetch(`${API_URL}/analysis/new`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ symbol, timeframe }) });
      const result = await res.json();
      if (!res.ok) throw new Error(result.detail || 'Analiz sırasında bir hata oluştu.');
  ...
  const handleConfirmTrade = useCallback(async (result) => {
    ...
    try {
      const openRes = await fetch(`${API_URL}/positions/open`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ symbol: result.symbol, recommendation: result.recommendation, timeframe: result.timeframe, price: result.data.price }) });
      const openResult = await openRes.json();
      if (!openRes.ok) throw new Error(openResult.detail || 'Pozisyon açılamadı.');
  ...
  const handleRunScan = useCallback(async () => {
    ...
    try {
        const res = await fetch(`${API_URL}/scanner/run`, { method: 'POST' });
        const result = await res.json();
        if(!res.ok) throw new Error(result.detail || 'Tarama başlatılamadı.');
  ...
  const handleSaveSettings = useCallback(async (newSettings) => {
      ...
      try {
          ...
          const result = await res.json();
          if(!res.ok) throw new Error(result.detail || 'Ayarlar kaydedilemedi.');
  ...
  const handleClosePosition = useCallback(async (symbol) => {
    ...
    try {
      const res = await fetch(`${API_URL}/positions/${encodeURIComponent(symbol)}/close`, { method: 'POST' });
      const result = await res.json();
      if (!res.ok) throw new Error(result.detail || 'Pozisyon kapatılamadı.');
      showToast(result.message, 'success');
      fetchData();
    } catch (err) {
      showToast(err.message, 'error');
  ...
  const handleRefreshPnl = useCallback(async (symbol) => {
    ...
    try {
        await fetch(`${API_URL}/positions/${encodeURIComponent(symbol)}/refresh-pnl`, { method: 'POST' });
        setTimeout(() => { fetchData(); setRefreshingSymbol(null); }, 2000);
    } catch(err) {
        showToast(err.message, 'error');
  ...
  const handleReanalyze = useCallback(async (symbol) => {
      ...
      try {
          const res = await fetch(`${API_URL}/positions/${encodeURIComponent(symbol)}/reanalyze`, { method: 'POST' });
          const result = await res.json();
          if(!res.ok) throw new Error(result.detail || 'Yeniden analiz sırasında bir hata oluştu.');
